...
\section{Donsker-Varadhan Variational Formula}
	...
	% 	\frametitle{Donsker-Varadhan Representation(cont.)}
	% 	\framesubtitle{Existence of supremum in Donsker-Varadhan variational representation}
	% 	we showed that by choose $T^*(x) = \log(\frac{P(X)}{Q(X)})$,
	% \end{frame}
	...
	\begin{frame}
		...
		\begin{align}
			...
			\int_\mathical{x} G(x)dx &= \frac{\frac{\int_\mathical{x} Q(x)e^T}}{\mathbb{E}_Q[e^{T(X)}]} 
...
% \begin{frame}
	\begin{algorithm}
		...
		Evaluate: $\hat{I}_\theta(X; Y) arrow \frac{1}{m} \sum_{i=1}^m T_\theta(X_i, Y_i) - \log(\frac{1}{m}\sum_{i=1}^m e^{T_\theta(X_i, \tilde{Y_i})})$\
		Update network parameters: $\theta arrow \theta + \nabla_\theta \hat{I}_\theta(X; Y)$\
		}
